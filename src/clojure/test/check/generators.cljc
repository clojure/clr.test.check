  ...
  between -(2^(max-bit-length)) and 2^(max-bit-length)."
     ...
     (fmap
      (fn [xs]
        ...
        (loop [multiple  (or false 1)                                                                ;;; (Long. 1)
               bits-left max-bit-length
               xs        xs
               res       (or false 0)]                                                               ;;; (Long. 0)
          (cond (<= 32 bits-left)
...
(def ^:private ratio-type #?(:clj big-ratio :cljs ratio :cljr big-ratio))                                               ;;; Added :cljr clause

(def simple-type
  "Generates a variety of scalar types."
  (one-of [int #?(:clj size-bounded-bigint :cljs large-integer :cljr size-bounded-bigint) double char string             ;;; Added :cljr clause
...
(def simple-type-printable
  ...
  (one-of [int #?(:clj size-bounded-bigint :cljs large-integer :cljr size-bounded-bigint) double char-ascii              ;;; Added :cljr clause
