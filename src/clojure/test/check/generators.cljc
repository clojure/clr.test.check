...
(ns clojure.test.check.generators
  ...
  (:require [#?(:default clojure.core :cljs cljs.core) :as core                        ;;; Change :clj to :default
             #?@(:cljs [:include-macros true])]   #?(:cljr clojure.string)             ;;; Add clojure.string
...
(defn ^:private resize-symbolish-generator
  ...
  (scale #(long (Math/Pow % 0.46)) g))                                         ;;; Math/pow
...
(defn
  ...
  (sized (fn [size]
           (bind (choose 1 5)
                 (fn [height] (core/let [children-size (Math/Pow size (/ 1 height))]            ;;; Math/pow
